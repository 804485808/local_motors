...
class News_review_model extends MY_Model{
     ...
     * @return array
     */
    public function getNewsReviewList($where,$limit){
        $res = $this->getNewsCommonLink('t1.*,t2.author,t2.title,t2.style,t3.username as visitor',array('wl_news_review'=>'t1'),array('News'=>'t2','Member'=>'t3'),$where,'t1.time desc',$limit);
        return $res;
    }

    public function updateReview($array,$where){
       if($this->db->update($this->mainTable,$array,$where)){
           return true;
       }else{
           return false;
       }
