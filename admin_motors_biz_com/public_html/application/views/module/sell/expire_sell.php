...
<td class="tab_nav">&nbsp;</td><td id="Tab3" class="tab_on"><a href="<?php echo site_url("module/sell/expire_sell2")?>">过期供应</a></td>
...
批量延长过期时间 <input type="text" size="3" name="days" id="days" value="60">天  
<input type="submit" value=" 确 定 " class="btn" onclick="if(Dd('days').value==''){alert('请填写天数');return false;}
if(Dd('days').value>=1){}else{alert('延期天数必须是正整数');return false;}
if(confirm('确定要延长'+Dd('days').value+'天吗？')){this.form.action='<?php echo site_url("module/sell/modify_sell/delay/")?>'}else{return false;}">
</span>

<input type="submit" value="刷新过期" class="btn" onclick="this.form.action='?moduleid=5&file=index&action=expire&refresh=1';">&nbsp;
