...
class Category_new_model extends MY_Model
{
    ...
    public function getShowCategory($type)
    {
        ...
        $sql = "select * from (select t1.catid,t1.catname,t2.title,t2.itemid,t2.tag,t2.thumb,t2.level as l2,t1.level as l1 from wl_category_new as t1 LEFT JOIN wl_news as t2 on t1.catid=t2.catid
           WHERE t1.parentid = '{$parentId}' and t1.status=1 and t2.status=3 ) as v ORDER BY v.l1  ASC, v.l2 ASC
          ";
       ...
       $re =  $query->result_array();
        if(!$re){
            $sql = "select * from (select t1.catid,t1.catname,t2.title,t2.itemid,t2.tag,t2.thumb,t2.level as l2,t1.level as l1 from wl_category_new as t1 LEFT JOIN wl_news as t2 on t1.catid=t2.catid
           WHERE t1.catid = '{$parentId}' and t1.status=1 and t2.status=3 ) as v ORDER BY v.l1  ASC, v.l2 ASC
          ";
