...
class Sell_model extends MY_Model{
    ...
    public function getLatestProducts($limit){
        ...
        $selllist = $this->getSellCommonLink('t1.username,t1.company,t1.itemid,t1.linkurl,t1.thumb,t1.title,t1.minamount,t1.unit,t1.minprice,t1.currency,t1.pptword,t2.areaname',array('wl_sell'=>'t1'),array('Area'=>'t2'),'t1.status=3',
            ' addtime desc','',$limit);
        return $selllist;
    ...
    public function getHotProducts($limit){
        $hotProducts = $this->getSellCommonLink('t1.username,t1.company,t1.itemid,t1.linkurl,t1.thumb,t1.title,t1.minamount,t1.unit,t1.minprice,t1.currency,t1.pptword,t2.areaname',array('wl_sell'=>'t1'),array('Area'=>'t2'),'t1.status=3',
            'hits desc','',$limit);
//        $hotProducts = $this->getSellCommon('username,company,itemid,linkurl,thumb,title,minamount,unit,minprice,currency','','hits desc',$limit);
        return $hotProducts;
    ...
    public function  getHotProductPrice($limit){
        $re = $this->getSellCommon('itemid,title,username,linkurl,pptword,currency,minprice',"status=3",'','hits desc',$limit);
        $this->load->model('category_option_model','category_option');
        foreach($re as $k=>$v){
    ...
    //产品首页价格
    public function  getHotCategoryPrice($limit){
        $re = $this->getSellCommon('itemid,title,username,linkurl,pptword,currency,minprice',"status=3 and minprice !='0.00'",'','hits desc',$limit);
        $this->load->model('category_option_model','category_option');
        foreach($re as $k=>$v){
            $rattr = $this->category_option->getSellOption($v['pptword'],$v['itemid']);
