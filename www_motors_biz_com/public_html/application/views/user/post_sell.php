...
function uploadSuccess(file, serverData) {
	try {
		...
		//console.log(json);
        if(json.code){
			if($('#'+this.customSettings.uploaded_path).val()==""){
			...
			}else if($('#'+this.customSettings.uploaded_path+"_1").val()==""){
				var $preview = $('#'+this.customSettings.uploaded_preview+"_1");
				var $image  = $('<img>');
				$image.attr('src', '<?php echo $site['image_domain']."/";?>'+json.src);
				$image.attr('height', 100);
				$image.attr('width', 100);
				$preview.html('').append($image);
				$preview.append("<br/><span id=\"deleteimg\"><a style=\"cursor:pointer;\" onclick=\"delete_image('_1')\"><font style='color:red'>delete image</font></a></span>");
				$('#'+this.customSettings.uploaded_path+"_1").val(json.src);
			}else if($('#'+this.customSettings.uploaded_path+"_2").val()==""){
				var $preview = $('#'+this.customSettings.uploaded_preview+"_2");
				var $image  = $('<img>');
				$image.attr('src', '<?php echo $site['image_domain']."/";?>'+json.src);
				$image.attr('height', 100);
				$image.attr('width', 100);
				$preview.html('').append($image);
				$preview.append("<br/><span id=\"deleteimg\"><a style=\"cursor:pointer;\" onclick=\"delete_image('_2')\"><font style='color:red'>delete image</font></a></span>");
				$('#'+this.customSettings.uploaded_path+"_2").val(json.src);
			}else{
				qmsg(this, 'You are only able to display 3 images max.');
			}
...
function add_group(){
	$.popup('Group Name : <input type="text" id="inputGroup" name="inputGroup" value="">', {modal:true, button:{ok:true,cancel:true},ok_callback:function(){
		if($("#inputGroup").val()!==undefined){
			...
			data:{tname:$("#inputGroup").val(),type:"addsell"},
			dataType:"json",
			success:function(data){
				if(data.code){
					$("#mycatid").append('<option selected value="'+data.tid+'" style="color:#999">'+data.tname+'</option>');
					$("#inputGroup").val('');
				}else{
					alert(data.message);
				}
...
<div class="user_main">
    ...
    <div class="inbox2">
		...
		<div class="Manage_title">Post a Product </div>
  ...
  <SELECT name="fourSelect" tabindex="4" class="column last" id="fourSelect" style="height: 214px; display: none;" size="10" ></SELECT>
			...
			<input class="input_P_3" name='minamount'/>
			<select style="width:130px;" name="unit" class="input_P_3a" id="unit">
				...
				<option value="Set" style="color:#000">Set</option>
			...
			<input class="input_P_3" name='minprice'/>
			<select name="currency" class="input_P_3a" id="currency">
			<option selected="" value="" style="color:#999">Currency</option>
		   ...
		   <div class="P_clear"></div>
			...
			<option selected="" value="<?php echo $v['tid']?>" style="color:#999"><?php echo $v['tname']?></option>
			<?php endforeach;?>
			</select>
			&nbsp;&nbsp;<a href="#mycatid" onclick="add_group()" id="addgroup">add group</a>
		   <div class="P_clear"></div>
