...
<div class="user_main">
    ...
    <div class="inbox2">
		...
		<div class="P_all_template">
			...
			<input type="hidden" id="itemid" name="itemid" value="<?php echo $news['itemid']?>" /> 
			<input class="input_P_1" name='title' id='title' value="<?php echo $news['title']?>"/>
			<span id="name_info" class="worry_msg" style="display:none"> 
                No news title
            </span>
			<div class="P_clear"></div>
			<div class="P_all_template_t">Category:</div>
			<option value="0" style="color:#999">Select Category</option>
			<select name="catid" class="input_P_3a" id="catid">
			<?php foreach($news_group as $v):?>
			<option <?php if ($news['catid']==$v['catid']):?>selected="selected"<?php endif;?> value="<?php echo $v['catid']?>" style="color:#999"><?php echo $v['catname']?></option>
			<?php endforeach;?>
			</select>
			<div class="P_clear"></div>
			 
			<div id="next">
			<div class="P_all_template_t"><b>*</b>News Detail:</div>
			<textarea name="content" id="content" cols="" rows="" class="input_P_2" style='width: 580px; height: 250px;'/><?php echo $news['news_data']['content']?></textarea>
			<div class="clear"></div>
			...
			<span id="spanButtonPlaceholder"></span><input type="hidden" id="uploaded_path" name="path" value="<?php echo $news['thumb']?>" /> 
			...
			<div class="P_all_upload_pic">
				<ul>
					<li><div id="uploaded_preview_panel"><img src="<?php echo $site['image_domain'].$news['thumb']?>" width="100" height="100"><br/><span id=\"deleteimg\"><a style="cursor:pointer;" onclick="delete_image('')"><font style='color:red'>delete image</font></a></span></div></li>
				</ul>
			</div>
			<div class="P_clear"></div>
			<div class="P_all_template_t">Introduce:</div>
			<textarea name="introduce" id="introduce" style="overflow:auto;font-size:12px;resize:none;width: 580px; height: 50px;"><?php echo $news['introduce']?></textarea>
			<div class="P_clear"></div>
			<div class="P_all_template_t">Author:</div>
			<input size=10 name='author' id='author' value="<?php echo $news['author']?>" />&nbsp;&nbsp;<span style="font-weight:bold;color: #666666;">Source:</span>&nbsp;<input size=18 name='source' id='source'  value="<?php echo $news['source']?>"/>&nbsp;&nbsp;<span style="font-weight:bold;color: #666666;">Source Link:</span>&nbsp;<input size=28 name='fromurl' id='fromurl'  value="<?php echo $news['fromurl']?>"/>
			<div class="P_clear"></div>
			<div class="P_all_template_t">Tag:</div>
			<input class="input_P_1" name='tag' id='tag' value="<?php echo $news['tag']?>"/>
