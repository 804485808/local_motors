...
class Uploadimg extends CI_Controller{
	function index(){
		...
		if($type == 'sell'){
			...
			$upload_path = $img_rootpath.'upload/user/'.$userid;
			if(!file_exists($upload_path)){
				$this->mkdirs($upload_path);
			}

			$pic_upload_path = $img_rootpath.'upload/user/'.$userid;
			...
			if($pic_count_1>1000){
				$err = array("code"=>0,"message"=>"Upload pictures more than 1000");
				$err = json_encode($err);
				echo $err;
				die();
			}
			...
			$config['wm_overlay_path'] = FCPATH."skin/cover.png";
			$config['quality'] = 90;
